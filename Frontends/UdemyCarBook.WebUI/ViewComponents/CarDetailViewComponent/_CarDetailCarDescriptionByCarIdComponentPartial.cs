...
namespace UdemyCarBook.WebUI.ViewComponents.CarDetailViewComponent
{
    public class _CarDetailCarDescriptionByCarIdComponentPartial:ViewComponent
    {
        ...
        public async Task<IViewComponentResult> InvokeAsync(int id)
        {
            ...
            var responseMessage = await client.GetAsync($"https://localhost:7008/api/CarDescriptions?id=" + id);
            if (responseMessage.IsSuccessStatusCode)
            {
                var jsonData = await responseMessage.Content.ReadAsStringAsync();
                var values = JsonConvert.DeserializeObject<ResultCarDescriptionByIdDto>(jsonData);
                return View(values);
            }
            return View();
        }
